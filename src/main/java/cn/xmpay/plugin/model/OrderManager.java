...
public class OrderManager {
            ...
            if (order != null) {
                ...
                plugin.getServer().getScheduler().runTask(plugin, () -> {
                    ...
                    if (p != null && p.isOnline()) {
                        ...
                        removeMapFromPlayer(p, order.getMapId(), order.getOriginalMainHand());
                    }
                });
    ...
    /**
     * 从玩家背包中移除指定ID的支付地图，并归还主手原物品
     */
    public void removeMapFromPlayer(org.bukkit.entity.Player player, int mapId,
                                    org.bukkit.inventory.ItemStack originalMainHand) {
        if (mapId < 0) return;
        ...
        if (plugin.getXMPayConfig().isDebug()) {
            plugin.getLogger().info("已从玩家背包移除支付地图并归还主手物品: " + mapId);
        }
    }
