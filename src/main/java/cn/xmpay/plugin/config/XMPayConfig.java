...
public class XMPayConfig {
    ...
    public boolean isPackagesEnabled() {
        return config.getBoolean("packages.enabled", true);
    }

    public List<PaymentPackage> getPackages() {
        List<PaymentPackage> packages = new ArrayList<>();
        List<?> list = config.getList("packages.list");
        if (list == null) return packages;

        for (Object obj : list) {
            if (obj instanceof ConfigurationSection section) {
                PaymentPackage pkg = PaymentPackage.fromSection(section);
    ...
    public static class PaymentPackage {
        ...
        public PaymentPackage(String id, String name, double amount, int bonus,
                              String requiredPermission, List<String> commands,
        ...
        public static PaymentPackage fromSection(ConfigurationSection section) {
            try {
                return new PaymentPackage(
                        section.getString("id", ""),
                        section.getString("name", "套餐"),
                        section.getDouble("amount", 0),
                        section.getInt("bonus", 0),
                        section.getString("required-permission", ""),
                        section.getStringList("commands"),
                        section.getString("icon", "GOLD_INGOT"),
